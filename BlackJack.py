...
def take_bet(chips):
    ...
    while True:
        try:
            chips.bet = int(input('Enter the number of chips you\'d like to bet: '))
        except ValueError:
            print('Chips should be integers only!')
        else:
            if(chips.bet>chips.total):
                print('Bets cannot exceed ',chips.total)
            else:
                break
...
def hit_or_stand(deck,hand):
    ...
    while True:
        hit_input = input("Would you like to hit or stand? Enter 'h' or 's' ")
        
        if(hit_input[0].lower()=='h'):
            hit(deck,hand)
        elif(hit_input[0].lower()=='s'):
